...
export function applySettingsPayload(
  ...
  if (data.advanced && typeof data.advanced === 'object') {
    ...
    if (['isolate', 'wasm', 'auto'].includes(a['dynamicToolSandbox'] as string)) config.tools = { ...(config.tools ?? {}), dynamicToolSandbox: a['dynamicToolSandbox'] as NonNullable<CascadeConfig['tools']>['dynamicToolSandbox'] };
...
export function settingsSnapshot(config: CascadeConfig): SettingsSnapshot {
  ...
  return {
    ...
    advanced: {
      ...
      dynamicToolSandbox: config.tools?.dynamicToolSandbox === 'worker' ? 'wasm' : config.tools?.dynamicToolSandbox,
