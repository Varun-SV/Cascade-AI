...
describe('/v1 routes', () => {
  ...
  // Held for good: the server remembered every tenant's state folder that
  // ever ran, and memory grew with them.
  it('lets go of the tenant\'s state folder when the run ends', async () => {
    h.held.clear();
    await post({ model: 'cascade', messages: [{ role: 'user', content: 'hi' }] });
